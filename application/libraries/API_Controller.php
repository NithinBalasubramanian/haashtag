...
class API_Controller {
     ...
     * @var array
     */
    protected $allowed_http_methods = ['get', 'delete', 'post', 'put', 'options', 'patch', 'head', 'header'];
    ...
    public function _APIConfig($config = []) {
        ...
        if (isset($config['key']))
            $configKey = $this->_api_key($config['key']);
    ...
    public function _limit_method(array $data) {
        ...
        if (!$this->CI->db->table_exists($this->API_LIMIT_TABLE_NAME)) {
        ...
        if ($limit_type == 'ip') {
            ...
            if ($this->CI->db->affected_rows() >= $limit_num) {
    ...
    private function _api_key(array $key) {
        ...
        if (strtolower($api_key_type) == 'header') {
            ...
            $api_key_header_name = $this->CI->config->item('api_key_header_name');
            ...
            if (isset($is_header['status']) === TRUE) {
                ...
                if ($api_key != "table") {
                    ...
                    if (!$this->CI->db->table_exists($this->API_KEYS_TABLE_NAME)) {
                        $this->_response(['status' => FALSE, 'message' => 'Create API Key Database Table'], self::HTTP_BAD_REQUEST);
                    }
                    $start = date('Y-m-d H:i:s');
                    $checkdate = date('Y-m-d H:i:s', strtotime('-60 minutes', strtotime($start)));
                    $where_key_data = [
                        'controller' => $this->CI->router->fetch_class(),
                        'api_key' => $HEADER_VALUE,
                        'used' => 'N',
                        'entry_date_time >=' =>  $checkdate,
                    ];

                    $limit_query = $this->CI->db->get_where($this->API_KEYS_TABLE_NAME, $where_key_data)->row_array();
                    ...
                    if (!$this->CI->db->affected_rows() > 0) {
                        ...
                        return $HEADER_VALUE;
                        // $this->CI->db->where($where_key_data)->update($this->API_KEYS_TABLE_NAME, array('used' => 'U'));
                    }
                }
            } else {
