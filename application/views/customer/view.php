...
<script>
    ...
    function  getReport()
    {
        ...
        var colNamesm = ["Sl no.","Customer Code","Name","Email Id","Mobile No","GST"
 ...
 {name: 'Sl no.', index: 'Sl_no', resizable: true},
    ...
    function get_report_url()
    {
        ...
        return  '<?php echo base_url('customer/getCustomerList'); ?>' + mData;
    }
    ...
    $(document).on("click", ".status", function () {
        ...
        $.ajax({
            method: "POST",
            dataType: "json",
            url: "<?php echo base_url('customer/updateStatus/CUST'); ?>",
...
$(document).on("click", ".view-dtls", function () {
    show_load("<?php echo lang('loading_customer_data_please_wait') ?>");
    var main_id = $(this).attr('main-attr');
    
     $.ajax({
            method: "POST",
            dataType: "json",
            url: "<?php echo base_url('customer/viewAjax'); ?>",
            data: {customer_id: main_id}
        }).done(function (data) {
            hide_load();
            if (data['status'] == true) {
