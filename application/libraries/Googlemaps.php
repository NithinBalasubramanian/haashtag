...
class Googlemaps {
    ...
    protected $adsense = FALSE;      // Whether Google Adsense For Content should be enabled
    protected $adsenseChannelNumber = '';       // The Adsense channel number for tracking the performance of this AdUnit
    protected $adsenseFormat = 'HALF_BANNER';   // The format of the AdUnit
    protected $adsensePosition = 'TOP_CENTER';    // The position of the AdUnit
    protected $adsensePublisherID = '';      // Your Google AdSense publisher ID
    protected $apiKey = '';       // If you've got an API key you can use it by passing this parameter. Setup an API key here: https://code.google.com/apis/console
    protected $backgroundColor = '';      // A hex color value shown as the map background when tiles have not yet loaded as the user pans
    protected $bicyclingOverlay = FALSE;     // If set to TRUE will overlay bicycling information (ie. bike paths and suggested routes) onto the map by default 
    protected $center = "37.4419, -122.1419";  // Sets the default center location (lat/long co-ordinate or address) of the map. If defaulting to the users location set to "auto"
    protected $class = '';      // A class name if wishing to style the map further through CSS. Can also be useful if wanting it to be responsive etc.
    protected $cluster = FALSE;     // Whether to cluster markers
    protected $clusterGridSize = 60;      // The grid size of a cluster in pixels
    protected $clusterMaxZoom = '';      // The maximum zoom level that a marker can be part of a cluster
    protected $clusterZoomOnClick = TRUE;      // Whether the default behaviour of clicking on a cluster is to zoom into it
    protected $clusterAverageCenter = FALSE;     // Whether the center of each cluster should be the average of all markers in the cluster
    protected $clusterMinimumClusterSize = 2;      // The minimum number of markers to be in a cluster before the markers are hidden and a count is shown
    protected $disableDefaultUI = FALSE;     // If set to TRUE will hide the default controls (ie. zoom, scale etc)
    protected $disableDoubleClickZoom = FALSE;     // If set to TRUE will disable zooming when a double click occurs
    protected $disableMapTypeControl = FALSE;     // If set to TRUE will hide the MapType control (ie. Map, Satellite, Hybrid, Terrain)
    protected $disableNavigationControl = FALSE;     // If set to TRUE will hide the Navigation control (ie. zoom in/out, pan)
    protected $disableScaleControl = FALSE;     // If set to TRUE will hide the Scale control
    protected $disableStreetViewControl = FALSE;     // If set to TRUE will hide the Street View control
    protected $draggable = TRUE;      // If set to FALSE will prevent the map from being dragged around
    protected $draggableCursor = '';      // The name or url of the cursor to display on a draggable object
    protected $draggingCursor = '';      // The name or url of the cursor to display when an object is being dragged
    protected $geocodeCaching = FALSE;     // If set to TRUE will cache any geocode requests made when an address is used instead of a lat/long. Requires DB table to be created (see documentation)
    protected $https = FALSE;     // If set to TRUE will load the Google Maps JavaScript API over HTTPS, allowing you to utilize the API within your HTTPS secure application 
    protected $navigationControlPosition = '';      // The position of the Navigation control, eg. 'BOTTOM_RIGHT'
    protected $infowindowMaxWidth = 0;      // The maximum width of the infowindow in pixels. Expecting an integer without units
    protected $keyboardShortcuts = TRUE;      // If set to FALSE will disable to map being controlled via the keyboard
    protected $jsfile = '';      // Set this to the path of an external JS file if you wish the JavaScript to be placed in a file rather than output directly into the <head></head> section. The library will try to create the file if it does not exist already. Please ensure the destination file is writeable
    protected $kmlLayerURL = '';      // A URL to publicly available KML or GeoRSS data for displaying geographic information. Multiple KML layers can be passed in by using an array of URL's. Note, if using multiple you'll probably have to set $kmlLayerPreserveViewport to true and manually set map center and zoom
    protected $kmlLayerPreserveViewport = FALSE;     // Specifies whether the map should be adjusted to the bounds of the KmlLayer's contents. By default the map is zoomed and positioned to show the entirety of the layer's contents
    protected $language = '';      // The map will by default load in the language of the browser. This can be overriden however here. For a full list of codes see https://spreadsheets.google.com/pub?key=p9pdwsai2hDMsLkXsoM05KQ&gid=1
    protected $loadAsynchronously = FALSE;     // Load the map and API asynchronously once the page has loaded
    protected $map_div_id = "map_canvas";    // The ID of the <div></div> that is output which contains the map
    protected $map_height = "450px";     // The height of the map container. Any units (ie 'px') can be used. If no units are provided 'px' will be presumed
    protected $map_name = "map";     // The JS reference to the map. Currently not used but to be used in the future when multiple maps are supported
    protected $map_type = "ROADMAP";    // The default MapType. Values accepted are 'HYBRID', 'ROADMAP', 'SATELLITE' or 'TERRAIN'
    protected $map_types_available = array();     // The other MapTypes available for selection on the map
    protected $map_width = "100%";     // The width of the map container. Any units (ie 'px') can be used. If no units are provided 'px' will be presumed
    protected $maps_loaded = 0;      // Counter which keeps track of how many maps have been created to avoid standard functions being output twice
    protected $mapTypeControlPosition = '';      // The position of the MapType control, eg. 'BOTTOM_RIGHT'
    protected $mapTypeControlStyle = '';      // The style of the MapType control. blank, 'DROPDOWN_MENU' or 'HORIZONTAL_BAR' values accepted.
    protected $minzoom = '';      // The minimum zoom level which will be displayed on the map
    protected $maxzoom = '';      // The maximum zoom level which will be displayed on the map
    protected $minifyJS = FALSE;     // If TRUE will run the JavaScript through Jsmin.php (this file and PHP5+ required) to minify the code
    protected $noClear = FALSE;     // If TRUE do not clear the contents of the map div
    protected $onboundschanged = '';      // The JavaScript action to perform when the viewport bounds have changed
    protected $oncenterchanged = '';      // The JavaScript action to perform when themap center property changes
    protected $onclick = '';      // The JavaScript action to perform when the map is clicked
    protected $ondblclick = '';      // The JavaScript action to perform when the map is double-clicked
    protected $ondrag = '';      // The JavaScript action to perform while the map is dragged	
    protected $ondragend = '';      // The JavaScript action to perform when the user stops dragging the map
    protected $ondragstart = '';      // The JavaScript action to perform when the user starts dragging the map
    protected $onidle = '';      // The JavaScript action to perform when the map becomes idle after panning or zooming
    protected $onload = '';      // The JavaScript action to perform when the map first loads. This library hi-jacks the window.load event so add any bespoke code using this option
    protected $onmousemove = '';      // The JavaScript action to perform when the user's mouse moves over the map container
    protected $onmouseout = '';      // The JavaScript action to perform when the user's mouse exits the map container
    protected $onmouseover = '';      // The JavaScript action to perform when the user's mouse enters the map container
    protected $onresize = '';      // The JavaScript action to perform when the maps div changes size
    protected $onrightclick = '';      // The JavaScript action to perform when the map is right-clicked
    protected $ontilesloaded = '';      // The JavaScript action to perform when the visible tiles have finished loading
    protected $onzoomchanged = '';      // The JavaScript action to perform when the maps zoom property changes
    protected $panoramio = FALSE;     // If TRUE will add photos from Panoramio as a layer to your maps as a series of large and small photo icons
    protected $panoramioTag = '';      // Restrict the set of Panoramio photos shown to those matching a certain textual tag
    protected $panoramioUser = '';      // Restrict the set of Panoramio photos shown to those matching a particular user
    protected $region = '';      // Country code top-level domain (eg "uk") within which to search. Useful if supplying addresses rather than lat/longs
    protected $scaleControlPosition = '';      // The position of the Scale control, eg. 'BOTTOM_RIGHT'
    protected $scrollwheel = TRUE;      // If set to FALSE will disable zooming by scrolling of the mouse wheel
    protected $sensor = FALSE;     // Set to TRUE if being used on a device that can detect a users location
    protected $streetViewAddressControl = TRUE;      // If set to FALSE will hide the Address control
    protected $streetViewAddressPosition = '';      // The position of the Address control, eg. 'BOTTOM'
    protected $streetViewControlPosition = '';      // The position of the Street View control when viewing normal aerial map, eg. 'BOTTOM_RIGHT'
    protected $streetViewCloseButton = FALSE;     // If set to TRUE will show the close button in the top right. The close button allows users to return to the aerial map
    protected $streetViewLinksControl = TRUE;      // If set to FALSE will hide the Links control
    protected $streetViewPanControl = TRUE;      // If set to FALSE will hide the Pan control
    protected $streetViewPanPosition = '';      // The position of the Scale control, eg. 'TOP_RIGHT'
    protected $streetViewPovHeading = 0;      // The Street View camera heading in degrees relative to true north. True north is 0, east is 90, south is 180, west is 270
    protected $streetViewPovPitch = 0;      // The Street View camera pitch in degrees, relative to the street view vehicle. Directly upwards is 90, Directly downwards is -90.
    protected $streetViewPovZoom = 0;      // The Street View zoom level. Fully zoomed-out is level 0, zooming in increases the zoom level.
    protected $streetViewZoomControl = TRUE;      // If set to FALSE will hide the Zoom control
    protected $streetViewZoomPosition = '';      // The position of the Scale control, eg. 'TOP_RIGHT'
    protected $streetViewZoomStyle = '';      // The size of the Street View zoom control. blank, 'SMALL' or 'LARGE' values accepted.
    protected $styles = array();     // An array of styles used to colour aspects of the map and turn points of interest on and off
    protected $stylesAsMapTypes = false;     // If applying styles, whether to apply them to the default map or add them as additional map types
    protected $stylesAsMapTypesDefault = '';      // If $stylesAsMapTypes is true the default style. Should contain the 'Name' of the style
    protected $tilt = 0;      // The angle of tilt. Currently only supports the values 0 and 45 in SATELLITE and HYBRID map types and at certain zoom levels
    protected $trafficOverlay = FALSE;     // If set to TRUE will overlay traffic information onto the map by default 
    protected $version = "3";      // Version of the API being used. Not currently used in the library
    protected $zoom = 13;      // The default zoom level of the map. If set to "auto" will autozoom/center to fit in all visible markers. If "auto", also overrides the $center parameter
    protected $zoomControlPosition = '';      // The position of the Zoom control, eg. 'BOTTOM_RIGHT'
    protected $zoomControlStyle = '';      // The size of the zoom control. blank, 'SMALL' or 'LARGE' values accepted.
    protected $markers = array();     // An array used by the library to store the markers as they are produced
    protected $markersInfo = array();     // An array containing marker information (id, latitude, longitude etc) for use elsewhere
    protected $polylines = array();     // An array used by the library to store the polylines as they are produced
    protected $polygons = array();     // An array used by the library to store the polygons as they are produced
    protected $circles = array();     // An array used by the library to store the circles as they are produced
    protected $rectangles = array();     // An array used by the library to store the rectangles as they are produced
    protected $overlays = array();     // An array used by the library to store the overlays as they are produced
    protected $directions = FALSE;     // Whether or not the map will be used to show directions
    protected $directionsStart = "";      // The starting location (lat/long co-ordinate or address) of the directions. Set to 'auto' to default it to the users location
    protected $directionsEnd = "";      // The destination point (lat/long co-ordinate or address) of the directions. Set to 'auto' to default it to the users location
    protected $directionsDivID = "";      // An element's ID on the page where textual directions will be output to. Leave blank if not required
    protected $directionsMode = "DRIVING";     // DRIVING, WALKING or BICYCLING (US Only) - The vehicle/mode of transport to show directions for
    protected $directionsAvoidTolls = FALSE;     // Whether or not directions should avoid tolls
    protected $directionsAvoidHighways = FALSE;     // Whether or not directions should avoid highways
    protected $directionsDraggable = FALSE;     // Whether or not directions on the map are draggable
    protected $directionsChanged = "";      // JavaScript to perform when directions are dragged
    protected $directionsUnits = "";      // 'metric' for kilometers and meters or 'imperial for miles and feet. Leave blank and it will default to the region or country of where directions are being obtained
    protected $directionsWaypointArray = array();     // An array of waypoints. eg array("Boston, MA", "Times Square, NY");
    protected $directionsWaypointsOptimize = FALSE;     // Should the waypoints be optimised? If TRUE, waypoints will be re-ordered to provide the most efficient route.
    protected $drawing = FALSE;     // Whether or not the drawing library tools will be loaded
    protected $drawingControl = TRUE;      // If set to FALSE will hide the Drawing Manager control
    protected $drawingControlPosition = 'TOP_CENTER';    // The position of the Drawing Manager control, eg. 'TOP_RIGHT'
    protected $drawingDefaultMode = 'marker';     // The default mode for the Drawing Manager. Accepted values are marker, polygon, polyline, rectangle, circle, or null. null means that the user can interact with the map as normal when the map loads, and clicks do not draw anything.
    protected $drawingModes = array();     // An array of modes available for use. Accepted values are marker, polygon, polyline, rectangle, circle
    protected $drawingOnComplete = array();     // An array of JS to execute when shapes are completed, one array element per shape. For example: array('circle'=>'JS here', 'polygon'=>'JS here');
    protected $drawingOnEdit = array();     // An array of JS to execute when shapes are changed/resized, one array element per shape. For example: array('circle'=>'JS here', 'polygon'=>'JS here');
    protected $places = FALSE;     // Whether or not the map will be used to show places
    protected $placesLocation = '';      // A point (lat/long co-ordinate or address) on the map if the search for places is based around a central point
    protected $placesRadius = 0;      // The radius (in meters) if search is based around a central position
    protected $placesLocationSW = '';      // If preferring to search within bounds the South-West position (latitude/longitude coordinate OR address)
    protected $placesLocationNE = '';      // If preferring to search within bounds the North-East position (latitude/longitude coordinate OR address)
    protected $placesTypes = array();     // The types of places to search for. For a list of supported types see http://code.google.com/apis/maps/documentation/places/supported_types.html
    protected $placesName = '';      // A term to be matched against when searching for places to display on the map
    protected $placesAutocompleteInputID = '';      // The ID attribute of the textfield that the autocomplete should effect
    protected $placesAutocompleteTypes = array();     // The types of places for the autocomplete to return. Options can be seen here https://developers.google.com/maps/documentation/javascript/places#places_autocomplete but include 'establishment' to only return business results, '(cities)', or '(regions)'
    protected $placesAutocompleteBoundSW = '';      // By specifying an area in which to search for Places, the results are biased towards, but not restricted to, Places contained within these bounds.
    protected $placesAutocompleteBoundNE = '';      // Both South-West (lat/long co-ordinate or address) and North-East (lat/long co-ordinate or address) values are required if wishing to set bounds
    protected $placesAutocompleteBoundsMap = FALSE;     // An alternative to setting the SW and NE bounds is to use the bounds of the current viewport. If set to TRUE, the bounds will be set to the viewport of the visible map, even if dragged or zoomed
    protected $placesAutocompleteOnChange = '';      // The JavaScript action to perform when a place is selected
    ...
    function getKey() {
        ...
        $key = $ci->UtilityModel->getSingColumData($db_ext . 'vendor_config', 'google_location_api', array('vendor_id' => $ci->session->userdata('vendor_id')));
        return $key;
    }
    ...
    function create_map() {
        ...
        if ($this->maps_loaded == 0) {
            if ($this->getKey() != "") {
                ...
                $apiLocation .= '://maps.googleapis.com/maps/api/js?key=' . $this->getKey() . '&';
            } else {
                if ($this->https) {
