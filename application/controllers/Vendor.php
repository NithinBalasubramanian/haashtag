...
class Vendor extends CI_Controller {
    ...
    public function __construct() {
     ...
     function viewAjax() {
        ...
        $main = array();
        $data['status'] = true;
        $main['edit'] = $this->UtilityModel->getRequiredData('S', $this->db_ext . 'vendor', '*', array('vendor_id' => $this->input->post('vendor_id')));
        $data['html']['title'] = $main['edit']['name'];
        if(empty($main['edit'])) {
           $data['status'] = false;  
            $data['message'] = lang('invalid_vendor');
        }
        $data['html'] = $this->load->view('vendor/ajax_view', $main, true);
    ...
    function view() {
        ...
        $this->data['head_link'] = array(
            array('link' => base_url('vendor/manage'), 'class' => 'btn editmain mkbtn btn-success', 'name' => '<i class="fa fa-plus icon-size"></i> Add', 'id' => ''),
        ...
        $this->data['breadcomb_title'] = 'Home <i class="fa fa-angle-double-right" aria-hidden="true"></i>  Vendor <i class="fa fa-angle-double-right" aria-hidden="true"></i> <span class="active-breadcomb">View</span>';

        $this->data['main_title'] = "Vendor";
        $this->data['status_url'] = base_url('vendor/updateStatus/CUST');


        $this->load->template('vendor/view', $this->data);
    }

    function manage($vendorId = "") {
        $this->data['message'] = $this->session->flashdata('message');
        $this->data['page_title'] = 'Vendor';
        $this->data['edit'] = array();
        $this->data['head_link'] = array(
            array('link' => base_url('vendor/view'), 'class' => 'btn editmain mkbtn btn-success', 'name' => '<i class="fa fa-eye icon-size"></i> View', 'id' => ''),
            array('link' => base_url('master/export_master/country'), 'name' => '<i class="fa fa-file icon-size"></i> Export', 'class' => 'btn mkbtn   btn-primary', 'id' => '', 'blank' => '1')
        );
        $this->data['breadcomb_title'] = 'Home <i class="fa fa-angle-double-right" aria-hidden="true"></i>  Vendor <i class="fa fa-angle-double-right" aria-hidden="true"></i> <span class="active-breadcomb">Manage</span>';

        $this->data['main_title'] = "Manage Vendor";

        if ($vendorId != "") {
            $this->data['edit'] = $this->UtilityModel->getRequiredData('S', $this->db_ext . 'vendor', '*', array('vendor_id' => $vendorId));
        ...
        $this->load->template('vendor/manage', $this->data);
    }
    ...
    public function getVendorList() {
        ...
        $result = $this->CommanModel->getVendorList($limit, $perpage);
        ...
        foreach ($result as $key => $row) {
            ...
            $row_data[$key]['cell'][] = $row['vendor_code'];
...
// $row_data[$key]['cell'][] = "  <a href='javascript:;' class='btn btn-info viewbank'   main-attr='" . $row['country_code'] . "' name-attr='" . $row['name'] . "' title='" . lang('edit') . "' >View All</a>";
            ...
            $editLink = "  <a href='" . base_url('vendor/manage/' . $row['vendor_id']) . "' class='btn btn-warning editmain'  name-attr='" . $row['name'] . "' title='" . lang('edit') . "' ><i class='fa fa-edit'></i></a>";

            $statusLink = "  <a href='javascript:;' class='btn " . $statusClass . " status' main-attr='" . $row['vendor_id'] . "' status-attr='" . $row['status'] . "'  name-attr='" . $row['name'] . "' title='" . lang('change_status') . "' >" . $statusHtml . "</a>";
            $viewLink = "  <a href='javascript:;' class='btn btn-primary view-dtls' main-attr='" . $row['vendor_id'] . "'  name-attr='" . $row['name'] . "' title='" . lang('view') . "' ><i class='fa fa-eye'></i></a>";
    ...
    function updateStatus($type) {
        ...
        if ($type == 'CUST') {
            ...
            $condition = array('vendor_id' => $this->input->post('id'));
            $table = $this->db_ext . 'vendor';
    ...
    function checkVendorCode() {
        $data['status'] = false;
        $data['message'] = 'Vendor Code is Required';

        $condition['vendor_code'] = $this->input->get_post('vendor_code');
        if ($this->input->get_post('edit_id') != "") {
            $condition['vendor_id !='] = $this->input->get_post('edit_id');
        }

        $table = $this->db_ext . 'vendor';
        $vendorDtls = $this->UtilityModel->getRequiredData('S', $table, '*', $condition);

        if (empty($vendorDtls)) {
            $data['message'] = 'Vendor code is available';
            $data['status'] = true;
        } else {
            $data['message'] = 'Vendor code already exists';
        }
    ...
    public function manageVendor() {
        ...
        if ($procced == 1) {
            $post['vendor_code'] = $this->input->post('vendor_code');
            if ($post['vendor_code'] == "") {
                $procced = 0;
                $data['message'] = lang('vendor_code_is_required');
            } else {
                $condition['vendor_code'] = $post['vendor_code'];
                if ($this->input->post('edit_id') != "") {
                    $condition['vendor_id !='] = $this->input->post('edit_id');
                }
                $vendorDtls = $this->UtilityModel->getRequiredData('S', $table, '*', $condition);
                if (!empty($vendorDtls)) {
                    $procced = 0;
                    $data['message'] = lang('vendor_code_already_assigned');
                }
            }
        }
        if ($procced == 1) {
        ...
        if ($procced == 1) {
            ...
            if (isset($_FILES['image']) && $_FILES['image']['name'] != '') {
                ...
                $config['upload_path'] = dirname($_SERVER["SCRIPT_FILENAME"]) . '/assets/uploads/vendor/';
        ...
        if ($procced == 1) {
            ...
            if ($status) {
                ...
                $contactTable = $this->db_ext . "vendor_contact";
                $addressTable = $this->db_ext . "vendor_address";
                $bankTable = $this->db_ext . "vendor_bank";
                //Insert Contact
                ...
                $this->UtilityModel->deleteThisData($contactTable, array('vendor_id' => $vendor_id));
                foreach ($name as $mk => $n) {
                    ...
                    $contactPost['vendor_id'] = $vendor_id;
                ...
                $this->UtilityModel->deleteThisData($addressTable, array('vendor_id' => $vendor_id));
                foreach ($country_code as $mk => $n) {
                    ...
                    $addressPost['vendor_id'] = $vendor_id;
                ...
                $this->UtilityModel->deleteThisData($bankTable, array('vendor_id' => $vendor_id));
                foreach ($name as $mk => $n) {
                    $bankPost['name'] = $n;
                    $bankPost['bank'] = $bank[$mk];
                    $bankPost['account_number'] = $account_no[$mk];
                    $bankPost['ifsc_code'] = $ifsc_code[$mk];
                    $bankPost['vendor_id'] = $vendor_id;
                ...
                $data['status'] = "1";
                $data['redirect_url'] = base_url('vendor/view');
                $data['message'] = $message;
                $this->UtilityModel->insertThisLog("Vendor", $message, $updatedData, $vendor_id);
            } else {
